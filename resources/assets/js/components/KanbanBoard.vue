<template>
    ...
    <div class="flex px-4 pb-8 items-start">
        <div v-for="group in groups" :key="group.id"
            ...
            <div class="text-sm mt-2 overflow-y-scroll h-80 px-2 mb-2">
                <vue-draggable :options="draggableOptions.stories" :list="group.stories" class="h-full">
                    <div class="card" :style="cardBorderStyle(group)" v-for="story in group.stories" :key="story.id">
                        ...
                        <div class="text-grey-darker mt-2 ml-2 flex justify-between items-end">
                            ...
                            <img :src="`https://randomuser.me/api/portraits/med/men/${story.id}.jpg`" width="30" height="30"
                                 class="rounded-full"/>
                        </div>
                    </div>
                </vue-draggable>
            ...
            <a href="#"
               class="p-2 text-grey-dark no-underline hover:underline hover:text-grey-darkest static h-4 clearfix hover:bg-grey rounded-b w-full h-full pin-b inline-block">
                Add a card ...
            </a>
        </div>
    </div>
